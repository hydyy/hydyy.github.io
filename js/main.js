...
function swiftFormat(handledObj){
	...
	handledObj.forEach((item,index,array)=>{
		...
		var annotation = localStorage.getItem('addComment') == 'true' ? '///\n' : '';
		var optional = localStorage.getItem('swift_optional') == 'true' ? '///\n' : '';
		propertyFormat += annotation + 'var {0}: {1}' + (optional ? '?' : '') + '\n'.format(key,value);
	})
	return propertyFormat;
}
...
var vm2 = new Vue({
	...
	data: {
		...
		swift_optional_checked: localStorage.getItem('swift_optional') == 'true' ? 1 : 0,
		languages: [
			'Java',
			'PHP',
			'Objective-C',
